...
impl<const OSC_RAW_BUF_SIZE: usize> Parser<OSC_RAW_BUF_SIZE> {
    ...
    /// no partial UTF-8 codepoint is buffered. Backs
    /// [`crate::parser::Parser::is_ground`] (ADR 0041 step 5); upstream has
    /// no equivalent — this state is otherwise private to `vte`.
    ///
    /// vte holds `State::Utf8` for the whole time a codepoint is mid-parse
    /// (`VtUtf8Receiver` resets it to `Ground` only once the codepoint
    /// completes or errors out), so the VT-state check below likely already
    /// subsumes the UTF-8 one. Both are still checked — belt and braces
    /// against a future vendored bump changing that — and the `debug_assert`
    /// catches it if the subsumption ever stops holding.
    #[inline]
    pub(crate) fn is_ground(&self) -> bool {
        let vt_ground = matches!(self.state, State::Ground);
        let utf8_ground = self.utf8_parser.is_ground();
        debug_assert!(
            !vt_ground || utf8_ground,
            "vte state is Ground but the vendored utf8 parser has a partial codepoint buffered",
        );
        vt_ground && utf8_ground
    }
    ...
    fn perform_action<P: Perform>(&mut self, performer: &mut P, action: Action, byte: u8) {
        match action {
            ...
            Action::OscPut => {
                // FORK: upstream caps this buffer only on its (previously
                // active, arrayvec-backed) `no_std` arm; the cap is the
                // resource bound and must not depend on the container, so
                // it is unconditional here. Semantics identical to
                // upstream's `is_full()` early-return.
    ...
    fn exceed_max_buffer_size() {
        ...
        match &dispatcher.dispatched[0] {
            Sequence::Osc(params, _) => {
                ...
                // FORK: upstream's `no_std`-arm assertion — the cap is
                // unconditional here (see Action::OscPut).
