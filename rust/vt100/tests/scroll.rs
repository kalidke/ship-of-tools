...
#[test]
fn scrollback() {
    let mut parser = vt100_ctt::Parser::new(24, 80, 10);
...
fn edge_of_screen() {
    ...
    // Upstream also pinned this screen's escape-sequence rendering, both
    // absolute and as a diff. The fork moves state by checkpoint, so the
    // round trip is what has to hold at the bottom-right corner.
