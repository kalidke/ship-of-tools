...
case_active_and_idle_frontends_print_their_state() {
    # topology plan §F step 1: a client with a self-reported declared
    # `name` (plus `role`/`host`) gets its own "role@host ... active|idle"
    # row instead of the generic "client <id>" one — the declared handle
    # itself is dropped from the label, `role@host` is what answers "which
...
case_version_names_the_daemons_own_host() {
    # topology plan §F step 1: `DaemonVersion.host` -- the endpoint row's
    # build cell must name the daemon's OWN declared host (distinct from
    # any client's `.host`), so "which frontend/daemon am I on" is
    # answerable from this one command without reading a log.
    stage_reply "version.query" '{"v":1,"id":2,"kind":"res","op":"version.query","payload":{"daemon":{"app_version":"0.6.0-dev+abc1234","protocol":1,"lane_build":"abc1234def","lane_proto":1,"host":"hub-host"},"clients":[]}}'
    stage_reply "workspace.list" '{"v":1,"id":3,"kind":"res","op":"workspace.list","payload":{"workspaces":[]}}'
    start_stub_daemon
    run_version
    stop_stub_daemon
    ...
    [ "$VER_RC" -eq 0 ] || { echo "  expected exit 0, got $VER_RC. Output:\n$VER_OUT"; return 1; }
    contains "$VER_OUT" "host hub-host" || { echo "  missing the daemon's own declared host: $VER_OUT"; return 1; }
    return 0
}

case_version_daemon_predating_host_field_prints_a_placeholder() {
    # `#[serde(default)]` on the Rust side: a daemon predating topology
    # plan §F step 1 answers version.query with no `.daemon.host` at all --
    # must still exit 0 with a placeholder, never crash on the missing key.
    ...
    [ "$VER_RC" -eq 0 ] || { echo "  expected exit 0, got $VER_RC. Output:\n$VER_OUT"; return 1; }
    contains "$VER_OUT" "host ?" || { echo "  expected a host placeholder for a daemon predating the field: $VER_OUT"; return 1; }
    return 0
}
