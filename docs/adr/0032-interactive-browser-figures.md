...
stream including the upgrade. A backend-local FE reaches `127.0.0.1:1241` directly; remote FE via `-L 1241:127.0.0.1:1241` (once the launcher forwards it). Same URL, both paths.
...
Headless proof on backend loopback: the server **binds 1241 and serves HTTP 200** — a
